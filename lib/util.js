...
ex) "name=zino; email=naver" 와 같이 쿠키값이 구성되어 있을 경우 만약 이름을 
     가져오고 싶다면, key값에 namez 이라면 문자열을 전달하면 된다..
------------------------------------------*/
function getCookie(key){
    let arr=document.cookie.split("; "); // [ "name=지노",  " email=daum"  ]

    for(let i=0;i<arr.length;i++){
        let kv=arr[i].split("="); //  ["name", "지노"]  
        if(kv[0]==key){
            return kv[1];
        }
    }
...
//좀더 짧고 세련된 방식 
/*
선언적 프로그래밍 - 예를 들어 반복문을 수행시 개발자로 하여금 반복의 과정을 제어하지 말고, 
                        오히려 무엇을 할지에 집중할수 있도록 지원하는 프로그래밍 방식..
                        전통적방식 반복문 수행시 개발자 몇부터~몇까지 돌려라....
                        선언적 프로그랭 방식에서는 개발자 프로그래밍 언어에게 반복해라!!
